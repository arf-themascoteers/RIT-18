...
class Reporter:
    def __init__(self, prefix, indices, folds):
        self.prefix = prefix
        ...
        self.details = np.zeros((len(self.indices), self.folds))
    ...
    def update_summary(self):
        score_summary = np.zeros((len(self.indices), 2))
        for index_index in range(len(self.indices)):
    ...
    def write_details(self):
        ...
        df.insert(0,"index",pd.Series(self.indices))
